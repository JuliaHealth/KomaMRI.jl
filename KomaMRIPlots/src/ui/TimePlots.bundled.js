...
async function mount(root, Plotly, request, response, overviewJSON) {
    ...
    range = first.layout.xaxis?.range?.length === 2 ? [
        ...first.layout.xaxis.range
    ] : [
        ...fullRange
    ];
    ...
    function setRange(next) {
        ...
        for (const plot of navigatorReady ? [
            graph,
            navigator
        ] : [
            graph
            ...
            if (!range.every((t, i)=>t === plot.layout.xaxis.range[i])) {
                ...
                if (plot === navigator) update['xaxis.rangeslider.range'] = [
                    ...fullRange
                ];
        ...
        fetchDetail();
        group?.dispatchEvent(new CustomEvent('koma-time-range', {
            detail: {
                source: root,
                range
            }
    ...
    async function draw(payload) {
        ...
        await Plotly.react(graph, payload.data, layout, {
            ...payload.config,
            responsive: true
        });
    }
    ...
    await draw({
        ...first,
        data: overview.data.map((trace, i)=>({
                ...trace,
                visible: visibility[i]
            }))
    });
    const fullYRanges = Object.fromEntries(Object.entries(graph._fullLayout).filter(([key])=>/^yaxis\d*$/.test(key)).map(([key, axis])=>[
            key,
            [
                ...axis.range
            ]
    ...
    if (hasSlider) {
        ...
        await Plotly.newPlot(navigator, overview.data.map((trace, i)=>({
                ...trace,
                visible: visibility[i],
                showlegend: false,
                hoverinfo: 'skip',
            ...
            height: 200,
            margin: {
                l: graph._fullLayout.margin.l,
                r: graph._fullLayout.margin.r,
                t: 0,
                b: 0
            },
    ...
    graph.on('plotly_relayout', async (event)=>{
        ...
        else if (event['xaxis.range[0]'] !== undefined || event['xaxis.range[1]'] !== undefined) next = [
            event['xaxis.range[0]'] ?? range[0],
            event['xaxis.range[1]'] ?? range[1]
        ];
    ...
    graph.on('plotly_restyle', ()=>{
        ...
        visibility = next;
        if (navigatorReady) Plotly.restyle(navigator, {
            visible: visibility
        });
        fetchDetail();
    });
