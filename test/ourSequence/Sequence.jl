...
NBlocks += 1
i = 334539
for n in 1:100 #while (i + 2*5518) < 5510100, cannot run the entire thing  
    global i
    global GRE
    ...
    GRE += Sequence(Grads,Rfs)
    global NBlocks += 1
    xGrad = [Grad(Gx[i+2578],324*dt,50*dt,115*dt,0) Grad(Gx[i+3152],335*dt,125*dt) Grad(Gx[i+3152+335+125*2],335*dt,125*dt) Grad(Gx[i+3152+335*2+125*4],335*dt,125*dt) Grad(Gx[i+3152+335*3+125*6],335*dt,125*dt,256*dt,0);]
...
#savefig(plot_seqd(GRE, simParams=sims),"DiscreteSeqMid.png")
