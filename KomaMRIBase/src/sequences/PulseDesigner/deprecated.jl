...
function RF_sinc(B1, T, sys::Scanner; G=[0, 0, 0], Δf=0, a=0.46, TBP=4)
    ...
    gr = [gr1 gr2]
    rf = [RF(t -> sinc_waveform(t - T/2), T; delay=ζ, Δf) RF(0, 0)]
	return Sequence(gr, rf)
end
...
"""
    spiral = spiral_base(FOV, N, sys; S0=sys.limits.Smax*2/3, Nint=8, λ=Nint/(2π*FOV), BW=60e3)
...
function spiral_base(
    ...
    S0=sys.limits.Smax*2/3, Nint=8, λ=Nint/(2π*FOV), BW=60e3
)
	kmax = N / (2*FOV)
...
function build_test_seq(; sys=Scanner())
    ...
    @addblock begin
        ...
        seq += build_adc(16; dwell=sys.limits.ADC_Δt, sys)
