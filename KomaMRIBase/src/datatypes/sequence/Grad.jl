...
+(x::Grad, y::Grad) = Grad(x.A .+ y.A, copy(max(x.T, y.T)), max(x.rise, y.rise), max(x.fall, y.fall), max(x.delay, y.delay), x.first + y.first, x.last + y.last) #TODO: solve this in a better way (by "stacking" gradients) issue #487
# Others
*(x::Grad, α::Real) = α * x
