...
function _imaging_raw_data(raw)
    profiles = filter(raw.profiles) do profile
        flags = profile.head.flags
        # Removing extra profile generated by JEMRIS, and NAV data which is not incorporated correctly in the recon
        flags != KomaMRICore.ISMRMRD_ACQ_IS_SURFACECOILCORRECTIONSCAN_DATA &&
            iszero(flags & KomaMRICore.ISMRMRD_ACQ_IS_NAVIGATION_DATA)
    end
    return RawAcquisitionData(raw.params, profiles)
end
