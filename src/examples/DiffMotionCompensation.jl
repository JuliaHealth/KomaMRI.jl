...
using MRIsim: γ, plot_grads, get_Bmatrix, get_SRmatrix, get_Mmatrix, 
...
##Plots
plot_grads_moments(DIF[1],title="Maxwell,M0-compensated, b=$(round(get_bvalue(DIF[1])*1e-6, digits=2)) s/mm2")
plot_grads_moments(DIF[2],title="Maxwell,M0,M1-compensated, b=$(round(get_bvalue(DIF[2])*1e-6, digits=2)) s/mm2")
plot_grads_moments(DIF[3],title="Maxwell,M0,M1,M2-compensated, b=$(round(get_bvalue(DIF[3])*1e-6, digits=2)) s/mm2")
plot_grads_moments(DIF[4],title="Maxwell,M0,M1,M2,M3-compensated, b=$(round(get_bvalue(DIF[4])*1e-6, digits=2)) s/mm2")
