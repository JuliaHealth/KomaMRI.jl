...
Base.isapprox(rf1::RF, rf2::RF) = begin
    return all(size(getfield(rf1,k)) == size(getfield(rf2,k)) for k ∈ fieldnames(RF)) && 
    all(all(isapprox.(getfield(rf1,k), getfield(rf2,k), atol=1e-7)) for k ∈ fieldnames(RF))
end

Base.isapprox(gr1::Grad, gr2::Grad) = begin
    return all(size(getfield(gr1,k)) == size(getfield(gr2,k)) for k ∈ fieldnames(Grad)) &&
    all(all(isapprox.(getfield(gr1,k), getfield(gr2,k), atol=1e-7)) for k ∈ fieldnames(Grad))
end

Base.isapprox(adc1::ADC, adc2::ADC) = begin
    return all(size(getfield(adc1,k)) == size(getfield(adc2,k)) for k ∈ fieldnames(ADC)) &&
        all(getfield(adc1,k) ≈ getfield(adc2,k) for k ∈ fieldnames(ADC))
end

Base.isapprox(s1::Sequence, s2::Sequence) = begin
    if length(s1) != length(s2)
        # println("Sequences do not have the same number of blocks.") # For debugging
        return false
    end
    # if s1.DEF != s2.DEF # Commented momentarily to work with extensions
    #     println("seq.DEF is not the same between sequences.")
    #     return false
    # end
    for i = 1:length(s1)
        if s1.ADC[i] ≉ s2.ADC[i]
            # println("seq[$i].ADC is not the same between sequences.") # For debugging
            return false
        end
        if all(s1.RF[:,i] .≉ s2.RF[:,i])
            # println("seq[$i].RF is not the same between sequences.") # For debugging
            return false
        end
        if all(s1.GR[:,i] .≉ s2.GR[:,i])
            # println("seq[$i].GR is not the same between sequences.") # For debugging
            return false
        end
        if s1.DUR[i] ≉ s2.DUR[i]
            # println("seq[$i].DUR is not the same between sequences.") # For debugging
            return false
        end
    end
    return true
...
function compress(data)
    ...
    return KomaMRICore.compress_shape(data)[2] #The function that was here did not work with the reader #For debbugging
end
...
function write_seq(seq::Sequence, filename)
    @info "Writing sequence $(basename(filename)) ..."
    ...
    ext_id = [id for id = seq.DEF["extension"]]
    blk_obj_dur_rf_gx_gy_gz_adc_ext = [[b, s, 0, rf_id[b], gx_id[b], gy_id[b], gz_id[b], adc_id[b], ext_id[b]] for (b, s) ∈ enumerate(seq)]
    ...
    open(filename, "w") do fid
        ...
        if !isempty(seq.DEF)
            ...
            for key ∈ sorted_keys
                val = seq.DEF[key]
                if !(key ∈ ["extension", "additional_text"]) # Temporary fix for extensions, REMOVE!
                    @printf(fid, "%s ", key)
                    if isa(val, String)
                        @printf(fid, "%s ", val)
                    else
        ...
        if !isempty(seq.DEF["additional_text"]) # Temporary fix for extensions, REMOVE!
            @printf(fid, "%s", seq.DEF["additional_text"])
            @printf(fid, "\n")
        end
        ...
        if !isempty(shape_data_id_num)
            ...
            @printf(fid, "[SHAPES]\n\n")
            for (data, id, num) ∈ sort(shape_data_id_num, lt = (x, y) -> (x[2] < y[2]))
