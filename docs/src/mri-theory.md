...
simulates the magnetization of each spin
...
The user can control the time between intermediate gradient samples with the parameter **Δt** (**Figure 2**).

The parameter **Δt_rf** manages the time between RF samples (**Figure 2**), and can be relatively large for 2D imaging where the slice profile is less relevant.
...
The **Bloch** method of Koma simulates the magnetization of each spin by solving the Bloch equations in the rotating frame:
```math
\begin{align} \tag{1}
