...
- If previewing pages with VitePress components such as tabs, run `npm run docs:build` before previewing. Serve the built static directory that contains `index.html` and `/assets/app.*.js` (typically `docs/build/1` after `npm run docs:build`), not the intermediate `docs/build/.documenter` tree. Serving the wrong root can load HTML while the client bundle is missing or under a different asset path, leaving Vue widgets non-interactive. Verify the page's referenced `/assets/app.*.js` returns 200.
