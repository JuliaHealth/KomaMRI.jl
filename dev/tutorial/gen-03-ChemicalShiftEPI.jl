...
p = [p1 p2] #hide
p.layout.fields[:xaxis2][:scaleanchor] = "y2" #hide
p.layout.fields[:xaxis1][:domain] = [0.0, 0.40] #hide
p.layout.fields[:xaxis2][:domain] = [0.58, 0.98] #hide
for axis in (:xaxis1, :yaxis1, :xaxis2, :yaxis2) #hide
    p.layout.fields[axis][:title] = "" #hide
    p.layout.fields[axis][:ticks] = "" #hide
    p.layout.fields[axis][:showticklabels] = false #hide
end #hide
for (trace, x) in zip(p.data, (0.43, 1.02)) #hide
