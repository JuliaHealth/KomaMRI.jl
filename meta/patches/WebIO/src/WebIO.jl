...
function prefetch_provider_file(basename)
    filepath = joinpath(get_webio_root_dir(), "src/providers", basename)
...
function __init__()
    ...
    # Updating paths to assets dynamically
    root_dir = get_webio_root_dir()
    global packagepath = normpath(joinpath(root_dir, "packages"))

    global PACKAGES_PATH = packagepath
    global BUNDLES_PATH = normpath(joinpath(root_dir, "deps/bundles"))
    global CORE_BUNDLE_PATH = joinpath(BUNDLES_PATH, "webio.bundle.js")
    global GENERIC_HTTP_BUNDLE_PATH = joinpath(BUNDLES_PATH, "generic-http.bundle.js")
    global MUX_BUNDLE_PATH = joinpath(BUNDLES_PATH, "mux.bundle.js")
    global BLINK_BUNDLE_PATH = joinpath(BUNDLES_PATH, "blink.bundle.js")
    
    global bundlepath = CORE_BUNDLE_PATH
