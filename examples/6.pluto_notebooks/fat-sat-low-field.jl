...
using MAT, Downloads, KomaMRI, PlutoPlotly

# ╔═╡ 1e70455c-5431-4a7e-bc5c-511b80e52ae9
md"""
Define the Sequence
"""

# ╔═╡ 536eff1a-1c0e-4c0a-a2b3-f465dbd87bb3
begin
	# Sequence parameters
    # TR = 4.9e-3   #RF Normal
...
# ╔═╡ 1a4d1157-a23c-4dcf-bffd-32c22a905b13
function FatSat(α, Δf; sample=false)
    RF_wf = matread(Downloads.download("https://github.com/cncastillo/KomaMRI.jl/raw/master/examples/5.fat_sat_low_field/GAUSS5120.mat"))["B1"]
    seq = Sequence()
...
# ╔═╡ b169a72a-af4e-4c1c-a00f-67929c385278
...
# ╔═╡ 7a6e92c5-c20b-45b4-9b87-9b6bbe476a76
md"""
Define the System
"""
...
md"""
Perform Simulation for `CMRA_signal_dynamics.jl` file
"""
...
# ╔═╡ f03616cf-c96b-48a3-bcbe-97a7d1729f6f
md"""
Define the Plots
"""
...
begin
    ...
    for i=eachindex(spin_group)
        p1 = scatter(x=t, y=Mxy(i), name=labs[i], legendgroup=labs[i], marker_color=cols[i])
        p2 = scatter(x=t, y=Mz(i), name=labs[i], legendgroup=labs[i], showlegend=false, marker_color=cols[i])#,line=attr(dash="dash"))
...
md"""
Perform Simulation for `CMRA_FatSat_flip_angle` file (this takes a long time ...)
"""

# ╔═╡ 0ef49f42-91ad-4066-a5c6-d6df0a31f6b0
begin
    # Simulation
    ...
    mag = zeros(ComplexF64, im_segments, Niso*4, length(param), length(offs))
    for (m, off) = enumerate(offs)
        for (n, FatSat_flip_angle) = enumerate(param)
            ...
            mag[:, :, n, m] = simulate(obj, seq, sys; sim_params)[end-im_segments+1:end, :]
        end
    end
end

# ╔═╡ 070459b6-3c40-4839-b6f7-95fe16328744
...
begin
    ...
    signal_myoc = reshape(mean(abs.(mean(mag[:, spins[1], :, :], 3)), 1), length(param), length(offs))
    signal_bloo = reshape(mean(abs.(mean(mag[:, spins[2], :, :], 3)), 1), length(param), length(offs))
    signal_fat  = reshape(mean(abs.(mean(mag[:, spins[3], :, :], 3)), 1), length(param), length(offs))
    signal_fat2 = reshape(mean(abs.(mean(mag[:, spins[4], :, :], 3)), 1), length(param), length(offs))
...
manifest_format = "2.0"
project_hash = "863a3d1841bc900d3b758a12e8c3d4f6ae20c65e"
...
git-tree-sha1 = "886aaaf98aba69e407aa86058259ce128d9b1f2f"
