...
S = [s[1] for s in signals] # Reshaping the simulated signals
bvals_SI = bvals .* 1e6 # Convert b-values from s/mm^2 to s/m^2

E_simulated   = abs.(S) ./ abs.(S[1])
...
layout = Layout( # hide
    title="PGSE Signal Attenuation E(b)", # hide
    xaxis=attr(title="b-value [s/mm^2]"), # hide
    # yaxis=attr(type="log") # hide
) # hide
