...
@testitem "Automatic differentiation tests" tags=[:core, :nomotion, :blochsimple, :ad, :enzyme, :reactant, :mooncake] begin
    ...
    run_skipci_ad_tests = isnothing(get(ENV, "CI", nothing))
    run_ad_probe(tags::Symbol...) =
        run_skipci_ad_tests && (run_all_ad_tests || test_group in tags)
    ...
    if run_ad_probe(:reactant, :enzyme)
        import Enzyme
        import Reactant
