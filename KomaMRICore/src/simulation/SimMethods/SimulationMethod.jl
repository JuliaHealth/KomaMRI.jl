...
function initialize_spins_state(
    obj::AbstractPhantom{T}, sim_method::SimulationMethod
) where {T<:Real}
    Nspins = length(obj)
    Mxy = zeros(T, Nspins)
    Mz = get_ρ(obj)
...
prealloc(sim_method::SimulationMethod, backend::KA.Backend, obj::AbstractPhantom{T}, M::Mag{T}, max_block_length::Integer, groupsize) where {T<:Real} = DefaultPrealloc{T}()
